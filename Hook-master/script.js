...
var ByPassTracerPid = function () {
    // 使用 Frida 的 Module.findExportByName 函数找到 fgets 的导出地址
    var fgetsPtr = Module.findExportByName('libc.so', 'fgets');
    // 使用 Frida 的 NativeFunction 类，定义 fgets 函数的返回类型为 pointer，参数类型为 pointer, int, pointer
    var fgets = new NativeFunction(fgetsPtr, 'pointer', ['pointer', 'int', 'pointer']);
    // 替换 fgets 的实现，以拦截读取 /proc/self/status 文件中的 TracerPid 字段，并将其值改为 0
...
function getStackTrace() {
    // 使用 Java 异常类 Exception 来生成堆栈跟踪
    var Exception = Java.use('java.lang.Exception');
    var ins = Exception.$new('Exception');
    // // 调用 Exception 实例的 getStackTrace 方法，获取当前线程的堆栈跟踪数组 straces
...
function get_format_time() {
    // 获取当前系统的日期和时间，并将其格式化为常见的 YYYY-MM-DD HH:MM:SS 格式
    var myDate = new Date();
...
//        arg: arg
//    };
    // send 函数通常用于在 Frida 脚本中发送数据到宿主应用或主脚本
    send({
        'type': 'notice',
        'time': _time,
        'action': action,
...
// 针对Mac地址的单独修改
function hookMethod_Special(targetClass, targetMethod, targetArgs, action, messages) {
    try {
        /// 尝试获取目标类的引用
        var _Class = Java.use(targetClass);
    } catch (e) {
...
// hook_Special方法(去掉不存在方法）
function hook_Special(targetClass, methodData) {
    try {
        var _Class = Java.use(targetClass);
    } catch (e) {
        return false;
    ...
    methodData.forEach(function (methodData) {
        for (var i in methods) {
            if (methods[i].toString().indexOf('.' + methodData['methodName'] + '(') != -1 || methodData['methodName'] == '$init') {
                hookMethod_Special(targetClass, methodData['methodName'], methodData['args'], methodData['action'], methodData['messages']);
                break;
            }
        }
    });
